    ...
    try {
        ...
        if (req.user.access_level)
        {
            if (req.user.access_level == 3) {
                ...
                let caregivers = await Admin.find({ parent_ID: req.user._id }).skip((perPage * page) - perPage).limit(perPage).exec();
                let count = await Admin.find({ parent_ID: req.user._id }).countDocuments().exec();
